		...
		for (var i = 0; i < selectOptionData.length; i++) {
				...
				.bind("mouseup.selectmenu", function(event) {
					if (self._safemouseup && !self._disabled(event.currentTarget) && !self._disabled($( event.currentTarget ).parents( "ul>li." + self.widgetBaseClass + "-group " )) ) {
						var changed = $(this).parent().data('index') != self._selectedIndex();
						self.index($(this).parent().data('index'));
						self.select(event);
						if (changed) {
							self.change(event);
						}
				...
				.bind('mouseover.selectmenu focus.selectmenu', function(e) {
					...
					if (!$(e.currentTarget).hasClass(self.namespace + '-state-disabled')) {
						...
						$(this).parent().removeClass('ui-state-active').addClass(self.widgetBaseClass + '-item-focus ui-state-hover'); 
					}
				})
				.bind('mouseout.selectmenu blur.selectmenu', function() { 
					...
					$(this).parent().removeClass(self.widgetBaseClass + '-item-focus ui-state-hover');
				});
